...
Твоя задача — выбрать 3–5 наиболее подходящих букета из базы и кратко объяснить, почему они подходят. Если параметров недостаточно, уточни запрос.
...
export class FlowersService {
  ...
  async askYaGptLite(prompt: string, context: string): Promise<any> {
    const data =
    {
      ...
      "completionOptions": {
        "stream": false,
        "temperature": 0.7, // по умолчанию было .6
        "maxTokens": "100",
        "reasoningOptions": {
          "mode": "DISABLED"
        }
